...
def run_conversions(old_subject_id, old_ses_id, abs_subject_folder, output_dir, info_list,
    ...
    "runs conversion for one subject for all modalities defined id info_list"
    ...
    for infodict in info_list:
        convert_modality(old_subject_id, old_ses_id, output_dir, bvecs_from_scanner_file=bvecs_from_scanner_file,
                         public_sub_id=None, face_dir=face_dir, **infodict)
...
def test_get_public_sub_id():
    assert (get_public_sub_id("lhab_abc2", "test_data/lut_test.tsv") == "lhabX0003", "Test failed")
